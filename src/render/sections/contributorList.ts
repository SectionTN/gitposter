...
export async function drawContributorList(ctx: SKRSContext2D, data: RepoData, theme: Theme, y: number, W: number): Promise<number> {
...
async function drawAvatar(ctx: SKRSContext2D, contrib: Contributor, cx: number, cy: number, r: number): Promise<void> {
  if (contrib.login) {
    try {
      const img = await loadImage(`https://avatars.githubusercontent.com/${contrib.login}?s=${r * 2}`)
      ctx.save()
      ctx.beginPath()
