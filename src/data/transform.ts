import type { RawData, RawCommit, DayBucket, Contributor, Milestone, Language, RepoData, Theme } from './model.js'
...
export function aggregateToDayBuckets(commits: RawCommit[]): DayBucket[] {
  ...
  sorted.forEach(c => {
    ...
    if (existing) {
      ...
      map.set(key, { date: startOfDay(c.date), count: 1, authors: [c.author.name] })
    }
  })
  ...
  while (cursor <= last) {
    ...
    result.push(map.get(key) ?? { date: new Date(cursor), count: 0, authors: [] })
...
export function buildLanguages(rawLangs: Record<string, number>): Language[] {
  ...
  if (otherPct > 0) {
    result.push({ name: 'Other', pct: Math.round(otherPct), color: LANGUAGE_COLORS.Other })
  }
...
export function buildRepoData(raw: RawData, theme: Theme): RepoData {
  ...
  const milestones: Milestone[] = raw.tags
    ...
    .map(t => ({ tag: t.name, date: t.date!, label: cleanMilestoneLabel(t.name) }))

  const languages = buildLanguages(raw.languages)

  const first = buckets.length > 0 ? buckets[0].date : new Date()
  const last = buckets.length > 0 ? buckets[buckets.length - 1].date : new Date()
  const totalDays = Math.max(1, Math.round((last.getTime() - first.getTime()) / 86_400_000) + 1)
